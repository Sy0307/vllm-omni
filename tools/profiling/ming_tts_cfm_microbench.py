...
def run_inplace_ode_update_compare(generator, inputs, args, llm_config, talker_cfg) -> dict:
    generator._pack_qkv_enabled = True
    generator._qkv_packed = False
    generator._cfm.use_preembedded_cfg = True
    generator._cfm.use_precomputed_temb = True
    ...
    def _set_enabled(enabled: bool) -> None:
        generator._cfm.use_inplace_ode_update = enabled
        generator._sampler_pools.clear()
    ...
    samples = {
        "out_of_place": {"llm_decode_ms": [], "cfm_step_ms": [], "collect_ms": []},
        "inplace_update": {"llm_decode_ms": [], "cfm_step_ms": [], "collect_ms": []},
    }
    for idx in range(args.repeats):
        for name, enabled in (("out_of_place", False), ("inplace_update", True)):
    ...
    return {
        "mode": "inplace_ode_update_compare",
        "model": {
            "llm_layers": llm_config.num_hidden_layers,
            "llm_hidden": llm_config.hidden_size,
            "cfm_steps": talker_cfg["steps"],
...
def main() -> None:
    ...
    parser.add_argument("--compare-inplace-ode-update", action="store_true")
    ...
    if args.compare_inplace_ode_update:
        summary = run_inplace_ode_update_compare(generator, inputs, args, llm_config, talker_cfg)
        text = json.dumps(summary, indent=2, sort_keys=True)
        if args.output:
            Path(args.output).write_text(text)
        print(text)

...
def merge_pipeline_deploy(
    ...
    # For pipelines that DO have inter-stage edges, require either the explicit
    # stage-payload processor ABI or a dedicated legacy per-step async producer
    ...
    if (
        ...
        and not any(
            ps.stage_payload_processor or ps.async_chunk_process_next_stage_input_func for ps in pipeline.stages
        )
    ):
        raise ValueError(

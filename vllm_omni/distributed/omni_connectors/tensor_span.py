...
"""Pure tensor-span helpers shared by connector and worker payload paths."""
...
def merge_tensor_spans(
    ...
    if incoming_tensor.device != existing_tensor.device or incoming_tensor.dtype != existing_tensor.dtype:
        incoming_tensor = incoming_tensor.to(
            device=existing_tensor.device,
            dtype=existing_tensor.dtype,
        )
    if incoming_start == existing_end:
        return (
            torch.cat([existing_tensor, incoming_tensor], dim=0),
            existing_start,
            incoming_end,
        )
    if incoming_start < existing_end:
...
__all__ = ["TensorSpan", "get_tensor_span", "merge_tensor_spans"]

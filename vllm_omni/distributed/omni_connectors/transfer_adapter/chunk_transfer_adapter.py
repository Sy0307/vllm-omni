...
class OmniChunkTransferAdapter(OmniTransferAdapterBase):
    ...
    def __init__(self, vllm_config: Any):
        ...
        # state must exist before it can observe an enqueued task.
        self._sender_state_lock = threading.RLock()
        self._sender_tokens: dict[str, object] = {}
    ...
    def save_async(
        ...
        with self._sender_state_lock:
            ...
            sender_token = self._sender_tokens.setdefault(external_req_id, object())
            task = {
    ...
    def _send_single_request(self, task: dict):
        ...
        with self._sender_state_lock:
            ...
            if sender_token is not None and self._sender_tokens.get(external_req_id) is not sender_token:
                logger.debug("Discarding stale queued chunk for aborted request %s", external_req_id)
                return
            self._send_single_request_locked(task)

    def _send_single_request_locked(self, task: dict):
    ...
    def cleanup_sender(self, external_req_id: str) -> None:
        ...
        """
        with self._sender_state_lock:
            # Invalidate already-enqueued work before clearing counters. A
            # later request may reuse the external id and start again at
            # chunk zero without an old background task repopulating state.

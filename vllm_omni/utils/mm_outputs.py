...
def _to_cpu(value):
    ...
    if isinstance(value, list):
        ...
        if (
            ...
            # Full-payload AR models commonly emit one tiny tensor per active
            # request. Coalesce compatible rows before D2H so a decode batch
            # pays for one synchronization instead of one per request, then
            # restore the original list structure as CPU views.
            packed_cpu = (
                torch.cat(
                    [item.detach() for item in value],
                    dim=0,
                )
                .to("cpu")
                .contiguous()
            ...
            for item in value:
                length = item.shape[0]
                output.append(packed_cpu.narrow(0, offset, length))
                offset += length
            return output

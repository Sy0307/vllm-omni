...
torch::Tensor fish_decode_kvcache_attn(torch::Tensor q,
  ...
  TORCH_CHECK(partial_m.is_cuda(), "partial_m must be a CUDA tensor");
  TORCH_CHECK(partial_l.is_cuda(), "partial_l must be a CUDA tensor");
  TORCH_CHECK(partial_acc.is_cuda(), "partial_acc must be a CUDA tensor");
...
TORCH_LIBRARY(vllm_omni_fish_kvcache_attn, m) {
  m.def(
      ...
      "Tensor partial_m, Tensor partial_l, Tensor partial_acc) -> "
      "Tensor(a!)");
}

...
class OfficialFusedAliasFreeActivation1d(AliasFreeActivation1d):
    ...
    def _load_extension(cls):
        ...
        if cls._extension_unavailable:
            return None
        try:
            from vllm_omni.model_executor.models.indextts2.s2mel.modules.alias_free_cuda import (
                load,
            )
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        try:
            ...
            logger.warning_once(
                "Official BigVGAN fused alias-free activation failed; disabling the extension",
                exc_info=True,
            )
            type(self)._extension_unavailable = True

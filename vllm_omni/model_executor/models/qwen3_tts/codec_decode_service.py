...
@dataclass
class CodecDecodeFuture:
    stats: PyTorchCodecDecodeServiceStats
    ...
    def wait(self) -> dict[int, torch.Tensor]:
        ...
        if self.done_event is not None and not self._waited:
            assert self.device is not None
            with torch.cuda.device(self.device):
                current_stream = torch.cuda.current_stream(self.device)
                current_stream.wait_event(self.done_event)
                for wav_row in self.result.values():
                    if wav_row.is_cuda:
                        wav_row.record_stream(current_stream)
            self._waited = True
        self.stats.future_wait_ns += time.perf_counter_ns() - wait_start_ns
        return self.result


@dataclass
class _QueuedCodecDecodeFuture:
    stats: PyTorchCodecDecodeServiceStats
    _done: threading.Event = field(default_factory=threading.Event)
    _result: dict[int, torch.Tensor] | None = None
    _exception: BaseException | None = None
    _done_event: torch.cuda.Event | None = None
    _device: torch.device | None = None
    _stream_waited: bool = False
    ...
    def wait(self) -> dict[int, torch.Tensor]:
        ...
        if self._done_event is not None and not self._stream_waited:
            assert self._device is not None
            with torch.cuda.device(self._device):
                current_stream = torch.cuda.current_stream(self._device)
                current_stream.wait_event(self._done_event)
                for wav_row in self._result.values():
                    if wav_row.is_cuda:
                        wav_row.record_stream(current_stream)
            self._stream_waited = True
        self.stats.future_wait_ns += time.perf_counter_ns() - wait_start_ns
        return self._result


@dataclass
class _QueuedDecodeJob:
    group_chunks: list[list[tuple[int, torch.Tensor]]]
    record_batch: DecodeBatchRecordCallback | None
    future: _QueuedCodecDecodeFuture
    submit_ns: int
...
class PyTorchCodecDecodeService:
    ...
    This MVP intentionally keeps execution synchronous. It centralizes the
    batching/profile boundary so the caller can later swap in a threaded or
    sidecar service without changing Code2Wav output semantics.
    """
...
class DynamicBatchingCodecDecodeService:
    """Threaded PyTorch codec decode service with a Triton-like queue delay.

    The worker merges multiple caller submissions that arrive inside a short
    queue window. It is intentionally opt-in because a synchronous single-thread
    Stage1 runner may not have concurrent submissions to batch.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._pending: deque[_QueuedDecodeJob] = deque()
        self._closed = False
        ...
        self._worker = threading.Thread(
            ...
            name="qwen3-codec-dynamic-batcher",
            daemon=True,
        )
        self._worker.start()
    ...
    def decode_group_chunks_async(
        self,
        group_chunks: Sequence[Sequence[tuple[int, torch.Tensor]]],
        *,
        record_batch: DecodeBatchRecordCallback | None = None,
    ) -> _QueuedCodecDecodeFuture:
        submit_start_ns = time.perf_counter_ns()
        normalized_group_chunks = [list(group_chunk) for group_chunk in group_chunks if group_chunk]
        future = _QueuedCodecDecodeFuture(stats=self.stats)
        ...
        with self._condition:
            if self._closed:
                future.set_exception(RuntimeError("Qwen3 codec dynamic batcher is shut down"))
                return future
    ...
    def _collect_jobs(self) -> list[_QueuedDecodeJob]:
        with self._condition:
    ...
    def _decode_jobs(self, jobs: list[_QueuedDecodeJob]) -> None:
        ...
        try:
            ...
            results_by_job: dict[int, dict[int, torch.Tensor]] = {id(job): {} for job in jobs}
            for global_index, wav_row in wav_rows_by_global_index.items():
                job, local_index = index_map[global_index]
                results_by_job[id(job)][local_index] = wav_row
            for job in jobs:
                job.future.set_result(
                    results_by_job[id(job)],
    ...
    def _merge_jobs(
        self,
        jobs: list[_QueuedDecodeJob],
    ) -> tuple[list[list[tuple[int, torch.Tensor]]], dict[int, tuple[_QueuedDecodeJob, int]]]:
        grouped_by_target_frames: dict[int, list[tuple[int, torch.Tensor]]] = {}
        index_map: dict[int, tuple[_QueuedDecodeJob, int]] = {}
        next_global_index = 0
        for job in jobs:
    ...
    @staticmethod
    def _first_record_batch_callback(jobs: list[_QueuedDecodeJob]) -> DecodeBatchRecordCallback | None:
        for job in jobs:
...
class AsyncCodecDecodeService:
    """Side-stream PyTorch codec decode backend.

    The service preserves the synchronous service's output contract but returns
    a Future so the caller can submit decode work first and only order the main
    stream on the decode result when waveform tensors are actually consumed.
    """
    ...
    def decode_group_chunks_async(
        ...
        *,
        ...
        if self._stream is None:
            result = self._sync_service.decode_group_chunks(
                group_chunks,
                record_batch=record_batch,
            )
            return CodecDecodeFuture(stats=self.stats, result=result)

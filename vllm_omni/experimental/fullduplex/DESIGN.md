...
Full-Duplex streaming is the primary driver for the typed Stage payload
contract below. The contract governs ordinary data-plane messages between two
pipeline stages and is shared by full-payload and async-chunk connectors. It is
not a Full-Duplex lifecycle API: abort, cancellation, session admission, and
resource leases remain on their reliable control planes.
...
### Verified Full-Duplex runtime contracts

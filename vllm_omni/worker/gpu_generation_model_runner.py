...
class GPUGenerationModelRunner(OmniGPUModelRunner):
    ...
    def _maybe_disable_mrope_for_code2wav(self) -> None:
        model_stage = getattr(self.model_config, "model_stage", None) or getattr(
            self.vllm_config.model_config, "model_stage", None
        )
        model_arch = getattr(self.model_config, "model_arch", None) or getattr(
            self.vllm_config.model_config, "model_arch", None
        )
        is_code2wav_stage = model_stage == "code2wav" or model_arch == "Qwen3TTSCode2Wav"
        if self.uses_mrope and is_code2wav_stage:

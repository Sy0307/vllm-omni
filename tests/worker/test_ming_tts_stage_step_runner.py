...
def test_ming_tts_step_runner_accepts_homogeneous_ming_batch():
    runner = SimpleNamespace(
        ...
        vllm_config=SimpleNamespace(model_config=SimpleNamespace(model_stage="ming_flash_omni_tts", enforce_eager=True)),
    )
    infos = [
...
def test_ming_tts_step_runner_run_and_commit_writes_per_request_audio():
    ...
    prepared = step_runner.prepare_step(request_ids=["r1", "r2"], runtime_infos=runtime_infos, inputs_embeds=inputs_embeds)
    step_runner.run_step(prepared=prepared, runner=runner)
    step_runner.commit_step(prepared=prepared, runner=runner)

    assert runner.model.audio_generator.calls == [((2, 3, 4), {"max_steps": 2, "cfg": None, "sigma": 0.25, "temperature": 0.0, "use_static_cache": True})]

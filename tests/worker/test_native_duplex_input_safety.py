...
def test_failed_preprocess_uses_embedding_interface_without_a_template():
    class Model:
        def __init__(self):
            self.calls = []
...
def test_native_async_lookahead_cannot_overwrite_unit_terminator(monkeypatch, terminator):
    ...
    state = model._minicpmo45_duplex_data_plane_helper.sessions["sid"]
    if terminator == 6:
        model.config = SimpleNamespace(gander_unit8=True)
        model._minicpmo45_duplex_data_plane_helper.sessions["req"] = state
        model._minicpmo45_native_duplex_token_ids_cache["interrupt_token_id"] = 6
    row = DuplexSamplingRow(0, "req", "sid", 1, {"is_speech": True}, 20)
    metadata = SimpleNamespace()
    calls = []

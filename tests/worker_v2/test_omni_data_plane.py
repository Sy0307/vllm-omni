...
def test_materialization_preserves_terminal_and_abort_fence(abort: bool) -> None:
    ...
    try:
        plane.register_request(_new_request())
        plane.reserve_outputs(["internal"])
        output = materializer.submit(Output(), plane)
        ...
        plane.drain_outputs()
        assert cleaned == ["internal"]
        assert len(batches) == (1 if abort else 2)
        if not abort:
            assert batches[0][0][1] == {"codes": {"audio": "chunk-0"}}
            assert not batches[0][0][0].is_finished()

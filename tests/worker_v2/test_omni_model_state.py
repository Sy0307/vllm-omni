...
def test_seeded_talker_mtp_uses_graph_with_precomputed_uniforms():
    state = _make_state(max_num_reqs=2)
    raw_calls = []
    graph_calls = []

    def raw_talker_mtp(*args, **kwargs):
        raw_calls.append(kwargs)
        return args[1], args[0].reshape(-1, 1)

    def graph_talker_mtp(*args, **kwargs):
        graph_calls.append(kwargs)
        return args[1], args[0].reshape(-1, 1)

    state.model.talker_mtp = raw_talker_mtp
    state._talker_mtp_runner = graph_talker_mtp
    sample_uniforms = torch.full((2, 1, 4), 0.5)
    generators = [torch.Generator().manual_seed(11), torch.Generator().manual_seed(22)]

    state._call_talker_mtp_with_sampling(
        torch.tensor([101, 202]),
        torch.zeros(2, 3),
        torch.zeros(2, 3),
        torch.zeros(2, 3),
    ...
    assert graph_calls[0]["sample_uniforms"].data_ptr() == sample_uniforms.data_ptr()
    assert "generators" not in graph_calls[0]


def test_talker_mtp_sample_uniforms_are_deterministic_per_request():
    state = _make_state(max_num_reqs=2)
    state._mtp_sample_uniforms = torch.empty((2, 2, 4))
    first_generators = [torch.Generator().manual_seed(11), torch.Generator().manual_seed(22)]
    second_generators = [torch.Generator().manual_seed(11), torch.Generator().manual_seed(22)]

    first = state._prepare_talker_mtp_sample_uniforms(first_generators, 2).clone()
    second = state._prepare_talker_mtp_sample_uniforms(second_generators, 2)
...
def test_forward_only_input_staging_preserves_rows_and_metadata(monkeypatch, second_dtype):
    state = _make_state(max_num_reqs=3)
    ...
    def move(tensors, device):
        ...
        return list(packed.split([tensor.shape[0] for tensor in tensors]))

    monkeypatch.setattr(state, "_batch_move_tensor_rows", move)
    with patch.object(type(state).__bases__[0], "prepare_inputs", return_value={}):
        output = state.prepare_inputs(batch, _DummyReqState())
    assert len(calls) == (1 if second_dtype == torch.int64 else 2)
    assert all(device == torch.device("cuda") for _, device in calls)
    rows = output["runtime_additional_information"]
    assert len(rows) == 2
    assert torch.equal(rows[0]["codes"]["audio"], second)
    assert torch.equal(rows[1]["codes"]["audio"], first)
    ...
    assert finished.device.type == "cpu"
    assert state.intermediate_buffer.buffers[1]["codes"]["audio"] is untouched


def test_prepare_inputs_keeps_ar_staging_in_preprocess(monkeypatch):
    state = _make_state(has_preprocess=True)
    state.model.batched_gpu_staging_keys = {("codes", "audio")}
    stage = MagicMock()
    monkeypatch.setattr(state, "_stage_batched_preprocess_inputs", stage)
    with patch.object(type(state).__bases__[0], "prepare_inputs", return_value={}):
        state.prepare_inputs(_DummyInputBatch([0]), _DummyReqState())
    stage.assert_not_called()


def test_staging_does_not_concatenate_different_source_devices(monkeypatch):
    state = _make_state(max_num_reqs=2)
    state.model.batched_gpu_staging_keys = {("codes", "audio")}
    state.intermediate_buffer.buffers[0] = {"codes": {"audio": torch.ones(2)}}
    state.intermediate_buffer.buffers[1] = {"codes": {"audio": torch.ones(3, device="meta")}}
    calls = []

    def move(tensors, device):
        assert len({tensor.device for tensor in tensors}) == 1

...
class FakeChunkAdapter:
    ...
    def is_done_receiving_chunks(self, request_id: str) -> bool:
        return request_id in self.finished_requests
...
class TestTerminalInputExecutionContract(unittest.TestCase):
    def _scheduler(self, *, native: bool, adapter_done: bool = False):
        scheduler = object.__new__(OmniGenerationScheduler)
        scheduler._native_data_plane = native
        scheduler.chunk_transfer_adapter = None if native else FakeChunkAdapter({"r1"} if adapter_done else set())
        scheduler.input_coordinator = SimpleNamespace(finished_requests={"r1"})
        return scheduler

    def test_native_does_not_finish_previous_output_when_terminal_only_arrived(self):
        scheduler = self._scheduler(native=True)
        scheduler_output = SimpleNamespace(input_terminal_req_ids=set())

        assert not scheduler._input_execution_is_terminal("r1", scheduler_output)

    def test_native_finishes_output_explicitly_tagged_as_terminal(self):
        scheduler = self._scheduler(native=True)
        scheduler_output = SimpleNamespace(input_terminal_req_ids={"r1"})

        assert scheduler._input_execution_is_terminal("r1", scheduler_output)

    def test_mrv1_adapter_keeps_done_receiving_chunks_semantics(self):
        scheduler = self._scheduler(native=False, adapter_done=True)
        scheduler_output = SimpleNamespace(input_terminal_req_ids=set())

...
def _summarize_session(
    *,
    ...
    return {
        ...
        "ok": returncode == 0 and result.get("ok") is True,
        ...
        "first_audio_delta_before_done": (
            bool(audio_delta_indices and done_indices) and min(audio_delta_indices) < min(done_indices)
        ),
...
async def _run_demo_process(
    *,
    ...
    if ref_audio:
        command.extend(["--ref-audio", str(ref_audio)])
...
async def run_pair(args: argparse.Namespace) -> dict[str, object]:
    ...
    summary_output = Path(args.summary_output) if args.summary_output else Path(validated["output_dir_a"]).parent / "summary.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--min-audio-deltas-per-session", type=int, default=0)

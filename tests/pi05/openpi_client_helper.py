...
"""Minimal OpenPI websocket client for pi0.5 online-serving tests."""
...
def require_dependencies() -> None:
    ...
    if missing:
        raise ModuleNotFoundError(f"pi0.5 OpenPI test dependencies are missing: {', '.join(missing)}")
...
def validate_session_result(
    ...
    *,
    ...
    if missing:
        raise AssertionError(f"Missing pi0.5 metadata keys: {missing}")
    ...
    if not metadata["needs_wrist_camera"]:
        raise AssertionError("pi0.5 test expects needs_wrist_camera=True")
    if metadata["needs_session_id"]:
        raise AssertionError("pi0.5 is stateless; needs_session_id must be False")

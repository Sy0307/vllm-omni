...
def test_indextts_attention_emits_padding_hint_only_for_explicit_padded_state(monkeypatch):
    ...
    assert padded_metadata.extra["mask_has_padding"] is True
    assert torch.equal(padded_metadata.attn_mask, mask[:, 0, 0, :])
    assert padded_metadata.unpad_data is unpad_data
...
def test_v25_default_recipe_selects_flashinfer_for_stage0():
    repo_root = Path(__file__).parents[4]

...
def test_build_mm_cpu_coalesces_compatible_tensor_list(monkeypatch):
    chunks = [
    ...
    result = build_mm_cpu({"codes.mel": chunks})
...
def test_build_mm_cpu_preserves_incompatible_tensor_list(monkeypatch):
    chunks = [
        torch.tensor([7], dtype=torch.long),
    ...
    def unexpected_cat(*args, **kwargs):
        raise AssertionError("incompatible tensor lists must not be coalesced")

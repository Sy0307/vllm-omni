...
def run_server(
    ...
    *,
    ...
    env = os.environ.copy()
    for name in ("MODEL_VERSION", "MODEL", "USE_GPT_LATENT", "DEPLOY_CONFIG", "PORT"):
        env.pop(name, None)
    ...
    if model is not None:
        env["MODEL"] = model
    if use_gpt_latent is not None:
        env["USE_GPT_LATENT"] = use_gpt_latent
...
@pytest.mark.parametrize(
    ("use_gpt_latent", "config_name"),
    [
        ("0", "indextts2_5.yaml"),
        ("1", "indextts2_5_latent.yaml"),
    ],
)
def test_version_2_5_selects_deploy_config(
    ...
    assert Path(argv[deploy_config_index]) == ROOT / f"vllm_omni/deploy/{config_name}"

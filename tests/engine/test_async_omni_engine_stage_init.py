...
def test_build_stage0_input_processor_uses_omni_renderer(monkeypatch):
    import vllm_omni.engine.stage_init_utils as init_mod
    ...
    monkeypatch.setattr(init_mod, "build_omni_renderer", lambda config: renderer)
    config = types.SimpleNamespace(model_config=types.SimpleNamespace(try_get_generation_config=lambda: {}))
    assert build_stage0_input_processor(config).renderer is renderer
...
def test_build_stage0_input_processor_does_not_resolve_tokenizer_when_skipped(monkeypatch):
    ...
    def unexpected_tokenizer(config):
        raise AssertionError("skip_tokenizer_init must not construct a tokenizer")

    monkeypatch.setattr(init_mod, "InputProcessor", DummyInputProcessor)
    monkeypatch.setattr(init_mod, "_build_token_only_renderer", lambda config: token_only_renderer)
    monkeypatch.setattr(init_mod, "build_omni_renderer", unexpected_tokenizer)
    config = types.SimpleNamespace(
        model_config=types.SimpleNamespace(skip_tokenizer_init=True, try_get_generation_config=lambda: {})
    )
    assert build_stage0_input_processor(config).renderer is token_only_renderer

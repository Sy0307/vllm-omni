...
1. normal audio input -> audio output or native listen -> response.done,
    ...
    def event_order_ok(self) -> bool:
        ...
        if "response.audio.delta" not in indices_by_type:
            listen_index = self.first_index(
                "response.listen",
                lambda event: isinstance(event.get("response"), dict)
                and isinstance(event["response"].get("metadata"), dict)
                and event["response"]["metadata"].get("model_listen") is True,
            )
            return listen_index is not None and indices_by_type["response.created"] < listen_index < indices_by_type[
                "response.done"
            ]
...
async def run_demo(args: argparse.Namespace) -> dict[str, object]:
    ...
    async with websockets.connect(url, max_size=64 * 1024 * 1024) as ws:
        ...
        try:
            ...
            await _wait_for(
                state,
                lambda: len(state.audio_deltas) > 0 or state.model_listen_count > 0 or state.done_count > 0,
                timeout_s=args.timeout_s,
                label="first audio or native listen",
            )
            first_response_spoke = len(state.audio_deltas) > 0
            if args.require_audio and not first_response_spoke:
                raise TimeoutError("Timed out waiting for first audio")
            ...
            if first_response_spoke and state.assistant_item_ids:
                await ws.send(
                    json.dumps(
                        {
            ...
            if first_response_spoke:
                ...
                await _wait_for(
                    ...
                    lambda: (
                        ...
                        or any(d.get("action") == "barge_in" for d in state.overlap_decisions)
                    ),
                    timeout_s=args.timeout_s,
    ...
    first_response_spoke = len(state.audio_deltas) > 0
    listen_only_response_ok = (
        not args.require_audio
        and not first_response_spoke
        and state.model_listen_count > 0
        and state.count("response.done") > 0
    )
    full_audio_response_ok = (
        first_response_spoke
    ...
    result = {
        ...
        and (full_audio_response_ok or listen_only_response_ok),
        "event_counts": {
        ...
        "listen_only_response_ok": listen_only_response_ok,
